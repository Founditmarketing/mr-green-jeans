...
export default function Safety() {
  const principles = [
    { num: '01', title: 'Stop-Work Authority', desc: "Every employee is empowered — and obligated — to stop work. No exceptions.", icon: <ShieldAlert size={24} color="var(--amber)" /> },
    { num: '02', title: 'Immediate Investigation', desc: 'We communicate, investigate, and share lessons learned instantly.', icon: <Eye size={24} color="var(--amber)" /> },
    { num: '03', title: 'Zero Delay Reporting', desc: 'All workplace incidents are reported immediately, period.', icon: <AlertTriangle size={24} color="var(--amber)" /> },
    { num: '04', title: 'Personal Accountability', desc: 'Every member owns their safety and the safety of their crew.', icon: <UserCheck size={24} color="var(--amber)" /> },
  ];
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '4rem', alignItems: 'center' }}>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
              ...
              <h1 className="section-title">ZERO COMPROMISE<br />ON <span style={{ color: 'var(--amber)', textShadow: '0 0 20px rgba(255,123,0,0.5)' }}>SAFETY.</span></h1>
              ...
              <div style={{ padding: '1.5rem', background: 'var(--dark2)', borderLeft: '4px solid var(--amber)', borderRadius: '8px' }}>
                <h4 style={{ fontFamily: 'Bebas Neue', fontSize: '2.5rem', color: 'var(--white)', marginBottom: '0.2rem', lineHeight: 1 }}>41,200+</h4>
                <p style={{ fontFamily: 'Barlow Condensed', fontSize: '1rem', color: 'var(--amber)', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 700 }}>Hours Without Incident</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {principles.map((p, idx) => (
                ...
                >
                  <div style={{ width: '44px', height: '44px', flexShrink: 0, background: 'rgba(255,123,0,0.1)', border: '1px solid rgba(255,123,0,0.3)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {p.icon}
                  </div>
                  <div>
