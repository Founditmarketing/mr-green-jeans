          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '4rem', alignItems: 'center' }}>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
              <div style={{ position: 'relative', borderRadius: '16px', overflow: 'hidden', aspectRatio: '4/3' }}>
                ...
                <div style={{ position: 'absolute', bottom: '1.5rem', right: '1.5rem', background: 'var(--amber)', padding: '1.2rem 1.5rem', borderRadius: '10px', zIndex: 3, boxShadow: '0 20px 40px rgba(0,0,0,0.6)' }}>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
              ...
              <h2 className="section-title" style={{ fontSize: 'clamp(2.5rem, 5vw, 3.5rem)', marginBottom: '1.5rem' }}>FAMILY OWNED.<br/><span style={{ color: 'var(--volt)', textShadow: '0 0 20px rgba(0,229,255,0.4)' }}>VALUES DRIVEN.</span></h2>
              ...
              <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2.5rem' }}>
                <li style={{ fontSize: '1.1rem', color: 'var(--text)', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                  <div style={{ width: '8px', height: '8px', background: 'var(--volt)', borderRadius: '50%', marginTop: '0.5rem', flexShrink: 0, boxShadow: '0 0 10px var(--volt)' }} />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1.5rem' }}>
                {[
                  { title: 'Safety First', icon: <ShieldCheck size={20} color="var(--volt)" />, desc: '0 Compromises' },
                  { title: 'People Focused', icon: <Users size={20} color="var(--volt)" />, desc: 'Elite Crews' },
                  { title: 'Mission Ready', icon: <Zap size={20} color="var(--volt)" />, desc: 'Storm Response' },
                  { title: 'Proven Results', icon: <Target size={20} color="var(--volt)" />, desc: '1,000+ Projects' }
                ].map((val) => (
                  <div key={val.title} style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', background: 'var(--dark2)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--glass-border)' }}>
                    <div style={{ width: '40px', height: '40px', background: 'rgba(0,229,255,0.08)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {val.icon}
                    </div>
                    <div>
                      ...
                      <p style={{ fontSize: '0.85rem', color: 'var(--volt)', marginTop: '0.2rem' }}>{val.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
